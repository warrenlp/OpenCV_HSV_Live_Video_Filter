...
#include "../include/TrackBarManager.h"
...
{
//	string url = "rtsp://admin:Eyeball11@192.168.1.154:554/play3.sdp";
	cv::VideoCapture cap("rtsp://admin:Eyeball11@192.168.1.154:554/play3.sdp"); // open the video file for reading
...
//		cap >> frame;
		...
		cv::cvtColor(frame, HSV_full, cv::COLOR_BGR2HSV_FULL);
		...
		if (ROIManager::drawRect())
		{
			...
			if (rect.area() > 0) 
			{
				...
				frameROI = frame(rect);
				cv::cvtColor(frameROI, HSV_ROI_full, cv::COLOR_BGR2HSV_FULL);
		...
		cv::cvtColor(histHue, histHue, cv::COLOR_GRAY2BGR);
		cv::cvtColor(histSat, histSat, cv::COLOR_GRAY2BGR);
		cv::cvtColor(histValue, histValue, cv::COLOR_GRAY2BGR);
